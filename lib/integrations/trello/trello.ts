...
export class TrelloAPI{
    ...
    async createBoard(token:string,name:string){
        const response = await fetch(`{${this.baseUrl}/boards?key=${this.apiKey}&token=${token}&name=${encodeURIComponent(name)}&defaultLists=true`,
            {method:'POST'}
        )
        if(!response.ok){
    ...
    async getBoardLists(token:string, boardId:string){
        const response = await fetch(`{${this.baseUrl}/boards/${boardId}/lists?key=${this.apiKey}&token=${token}`)
        if(!response.ok){
            throw new Error('Failed to fetch boards')
        }
        return response.json()
